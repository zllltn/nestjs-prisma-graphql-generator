  ...
  const dmmfDocument = new DmmfDocument(
    ...
    }),
    {
      ...
      blocksToEmit: getBlocksToEmit(
        parseStringArray(generatorConfig.emitOnly, 'emitOnly', ALL_EMIT_BLOCK_KINDS),
      ),
      contextPrismaKey: generatorConfig.contextPrismaKey ?? 'prisma',
    },
  );
  const emitTranspiledCode =
    parseStringBoolean(generatorConfig.emitTranspiledCode) ?? outputDir.includes('node_modules');
  // new project
