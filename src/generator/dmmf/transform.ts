...
import { FieldDefault, FieldDefaultScalar, FieldKind, ReadonlyDeep } from '@prisma/dmmf';
...
function transformModelField(dmmfDocument: DmmfDocument) {
  return (field: PrismaDMMF.Field): DMMF.ModelField => {
    ...
    // Casting to unknown first to bypass ReadonlyDeep and other type incompatibilities.
    return {
      ...field,
      type: field.type,
      location,
    ...
    return {
      ...
      fields: outputType.fields
        ...
        .map<DMMF.OutputSchemaField>((field) => {
          ...
          const argsTypeName =
            args.length > 0 ? `${typeName}${pascalCase(field.name)}Args` : undefined;
...
function selectInputTypeFromTypes(dmmfDocument: DmmfDocument) {
  return (inputTypes: readonly any[]): DMMF.SchemaArgInputType => {
    const { useUncheckedScalarInputs } = dmmfDocument.options;
    let possibleInputTypes: readonly any[];
    ...
    const selectedInputType =
      ...
      (useUncheckedScalarInputs &&
        possibleInputTypes.find((it) => (it.type as string).includes('Unchecked'))) ||
...
export function transformEnums(dmmfDocument: DmmfDocument) {
  return (enumDef: PrismaDMMF.DatamodelEnum | PrismaDMMF.SchemaEnum): DMMF.Enum => {
    ...
    // Fix for values: check if the values property exists and is an array
    let enumValues: Array<string | { name: string; dbName?: string | null }> = [];
    if ('values' in enumDef && Array.isArray(enumDef.values)) {
      enumValues = enumDef.values as Array<string | { name: string; dbName?: string | null }>;
    }
    ...
    return {
      ...
      valuesMap: enumValues.map((enumValue) => {
        ...
        return {
          value: enumValueName,
          name:
            (modelName && dmmfDocument.getModelFieldAlias(modelName, enumValueName)) ||
            enumValueName,
        };
      }),
    };
