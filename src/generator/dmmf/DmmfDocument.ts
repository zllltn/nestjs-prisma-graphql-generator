...
export class DmmfDocument implements DMMF.Document {
  ...
  constructor({ datamodel, schema, mappings }: PrismaDMMF.Document, public options: GeneratorOptions) {
    ...
    // Casting to any[] to bypass ReadonlyDeep incompatibility
    this.enums = (enumTypes as any[]).map(transformEnums(this));
    ...
    // Casting to any[] to bypass ReadonlyDeep incompatibility
    this.modelMappings = transformMappings(mappings.modelOperations as any[], this, options);
