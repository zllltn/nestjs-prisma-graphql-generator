...
function transformBareModel(model) {
    var _a;
    const attributeArgs = (0, helpers_2.parseDocumentationAttributes)(model.documentation, 'type', 'model');
    return {
        ...model,
        typeName: (_a = attributeArgs.name) !== null && _a !== void 0 ? _a : (0, helpers_1.pascalCase)(model.name),
        fields: [],
        docs: (0, helpers_1.cleanDocsString)(model.documentation),
    };
}
...
function transformModelField(dmmfDocument) {
    return (field) => {
        const attributeArgs = (0, helpers_2.parseDocumentationAttributes)(field.documentation, 'field', 'field');
        ...
        const fieldTSType = (0, helpers_1.getFieldTSType)(dmmfDocument, typeInfo, field.isRequired, false);
        const typeGraphQLType = (0, helpers_1.getTypeGraphQLType)(typeInfo, dmmfDocument, undefined, undefined, field.isId);
        const { output = false, input = false } = (0, helpers_2.parseDocumentationAttributes)(field.documentation, 'omit', 'field');
        return {
            ...field,
            type: field.type,
            location,
            ...
            fieldTSType,
            typeGraphQLType,
            docs: (0, helpers_1.cleanDocsString)(field.documentation),
...
function transformInputType(dmmfDocument) {
    return (inputType) => {
        const modelName = (0, helpers_1.getModelNameFromInputType)(inputType.name);
        const modelType = modelName
            ? dmmfDocument.datamodel.models.find((it) => it.name === modelName)
            : undefined;
        return {
            ...inputType,
            typeName: (0, helpers_1.getInputTypeName)(inputType.name, dmmfDocument),
            fields: inputType.fields
                ...
                const typeGraphQLType = (0, helpers_1.getTypeGraphQLType)(selectedInputType, dmmfDocument);
                const fieldTSType = (0, helpers_1.getFieldTSType)(dmmfDocument, selectedInputType, field.isRequired, true);
...
function transformOutputType(dmmfDocument) {
    return (outputType) => {
        const modelName = dmmfDocument.datamodel.models.find((it) => it.name === (0, helpers_1.getModelNameFromOutputType)(outputType.name))
            ? (0, helpers_1.getModelNameFromOutputType)(outputType.name)
            : undefined;
        ...
        return {
            ...
            fields: outputType.fields
                ...
                const fieldTSType = (0, helpers_1.getFieldTSType)(dmmfDocument, outputTypeInfo, isFieldRequired, false);
                const typeGraphQLType = (0, helpers_1.getTypeGraphQLType)(outputTypeInfo, dmmfDocument);
                const args = field.args.map((arg) => {
                    const selectedInputType = selectInputTypeFromTypes(dmmfDocument)(arg.inputTypes);
                    const typeGraphQLType = (0, helpers_1.getTypeGraphQLType)(selectedInputType, dmmfDocument);
                    const fieldTSType = (0, helpers_1.getFieldTSType)(dmmfDocument, selectedInputType, arg.isRequired, true);
                    return {
                        ...arg,
                        selectedInputType,
                        fieldTSType,
                ...
                const argsTypeName = args.length > 0 ? `${typeName}${(0, helpers_1.pascalCase)(field.name)}Args` : undefined;
                return {
                    ...field,
    ...
    return (mapping) => {
        ...
        const actions = Object.entries(availableActions)
            ...
            const argsTypeName = method.args.length > 0
                ? `${(0, helpers_1.pascalCase)(`${kind}${dmmfDocument.getModelTypeName(mapping.model)}`)}Args`
                : undefined;
            const outputTypeName = method.outputType.type;
            const actionResolverName = `${(0, helpers_1.pascalCase)(kind)}${modelTypeName}Resolver`;
            const returnTSType = (0, helpers_1.getFieldTSType)(dmmfDocument, method.outputType, method.isRequired, false, mapping.model, modelTypeName);
            const typeGraphQLType = (0, helpers_1.getTypeGraphQLType)(method.outputType, dmmfDocument, mapping.model, modelTypeName);
            return {
        ...
        return {
            ...
            modelTypeName,
            actions,
            collectionName: (0, helpers_1.camelCase)(mapping.model),
            resolverName,
        };
    };
}
function selectInputTypeFromTypes(dmmfDocument) {
    return (inputTypes) => {
        ...
        if (selectedInputType.location === 'enumTypes') {
            ...
            inputType = (0, helpers_1.getInputTypeName)(inputType, dmmfDocument);
        }
        return {
            ...selectedInputType,
            type: inputType,
...
function getMappedActionName(actionName, typeName, options) {
    ...
    switch (actionName) {
        case 'findUnique': {
            return (0, helpers_1.camelCase)(typeName);
        }
        case 'findMany': {
            return (0, pluralize_1.default)((0, helpers_1.camelCase)(typeName));
        }
        default: {
            return defaultMappedActionName;
        }
...
function transformEnums(dmmfDocument) {
    return (enumDef) => {
        ...
        return {
            ...enumDef,
            docs: 'documentation' in enumDef ? (0, helpers_1.cleanDocsString)(enumDef.documentation) : undefined,
            typeName,
    ...
    return (model) => {
        ...
        const relationFields = model.fields
            ...
            const argsTypeName = outputTypeField.args.length > 0
                ? `${model.typeName}${(0, helpers_1.pascalCase)(field.name)}Args`
                : undefined;
            return {
                ...field,
                outputTypeField,
